...
public abstract class BaseHouse implements House {
    ...
    public void addCat(Cat cat) {
        ...
        if (this.capacity <= this.cats.size()) {
            throw new NullPointerException(NOT_ENOUGH_CAPACITY_FOR_CAT);
        }
        cats.add(cat);
    }
    ...
    public String getStatistics() {
        ...
        if (!this.cats.isEmpty()) {
            build.append(this.cats.stream().map(Cat::getName).collect(Collectors.joining(" ")).trim());
        } else {
            build.append("none");
        }
        build.append("\n");
        build.append(String.format("Toys: %d Softness: %d", this.toys.size(), this.sumSoftness())).append("\n");
